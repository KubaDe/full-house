...
const test = base.extend<
  {},
  { invited: Account; inviting: Account; room: Room }
>({
  inviting: [
    async ({ browser }, use) => {
...
test.describe.serial("Invite user to room", () => {
  test("Inviting user sends invitation", async ({
    inviting,
    room,
    invited,
  }) => {
    ...
    await inviting.page
      .locator("[name='userEmail']")
      .fill(invited.testUserEmail);
    await inviting.page.locator("button[form='inviteUserForm']").click();
    await inviting.page.waitForResponse(
      "**/api/trpc/invitation.inviteUserToRoomMutation",
    );
  });
  ...
  test("Invited user receives invitation", async ({ invited, room }) => {
    ...
    await invited.page
      .getByTestId(`acceptInvitation.room_${room.stateHelpers.roomId}`)
      .click();
    await invited.page.getByTestId("confirmationModal.confirmAction").click();
    await invited.page.waitForResponse(
      "**/api/trpc/invitation.respondInvitationMutation",
    );
  });
  ...
  test("Invited user joins the room", async ({ invited, room }) => {
    ...
    await test
      .expect(invited.page.getByTestId("menu.room"))
      .toHaveText("Test room");
  });
});

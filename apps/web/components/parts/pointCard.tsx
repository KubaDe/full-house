...
} from "@/components/uiKit/background-gradient";
...
export const PointCard = ({
  ...
  const obverse = (
    <>
      {!special && (
        <p className="absolute left-2 top-1 text-sm font-semibold">{label}</p>
      )}
      {content}
      {!special && (
        <p className="absolute bottom-1 right-2 rotate-180 text-sm font-semibold">
          {label}
        </p>
      )}
    </>

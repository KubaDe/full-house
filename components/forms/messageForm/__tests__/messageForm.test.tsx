...
describe("MessageForm", () => {
  ...
  it("should call submit when form is valid", async () => {
    ...
    const { result } = renderHook(() => useMessageForm({ onSuccess, sessionId: testSessionId }));
    await waitFor(() => expect(result.current.formUI).toBeDefined());
    render(result.current.formUI);

    await userEvent.type(screen.getByPlaceholderText("Type a message"), "My message");
  ...
  it("should call onInvalid when form is invalid", async () => {
    ...
    const { result } = renderHook(() => useMessageForm({ onSuccess, onInvalid, sessionId: testSessionId }));
    await waitFor(() => expect(result.current.formUI).toBeDefined());
    render(result.current.formUI);
    ...
    expect(onInvalid).toHaveBeenCalledWith(
      expect.objectContaining({
        message: expect.objectContaining({ type: "too_small" }) as {},
      }),
      expect.anything(),
    );

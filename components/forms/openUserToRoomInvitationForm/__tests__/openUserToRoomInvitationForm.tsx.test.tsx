...
describe("InviteUserToRoomForm", () => {
  ...
  it("should enable open invitation when switching disabled invitation", async () => {
    ...
    const { result } = renderHook(() => useOpenUserToRoomInvitationForm({ onSuccess, roomId: testRoomId }));
  ...
  it("should display correct switch description", async () => {
    ...
    expect(
      await screen.findByText(/Allow to join the "Room 1" room for everybody with the link/),
    ).toBeVisible();
  });
});

...
import { SwitchCard } from "@/components/uiKit/switchCard";
  ...
  const onEnabledChange = useCallback(
    (value: boolean) => {
      switchOpenInvitation(
        ...
        {
          onError: async (error) => {
            toast.error(error.message ?? "Failed to update open invitation", { richColors: true });
            await utils.invitation.roomOpenInvitationQuery.invalidate();
            onError?.();
          },
          onSuccess: async () => {
            toast.success(
              value ? "Open invitation created successfully!" : "Open invitation disabled successfully!",
            );
            await utils.invitation.roomOpenInvitationQuery.invalidate();
            onSuccess?.();
          },
    ...
    [onError, onSuccess, roomId, switchOpenInvitation, utils.invitation.roomOpenInvitationQuery],
  );
  ...
  const formUI = (
    <form className="space-y-4">
      <SwitchCard
        ...
        subtitle={
          roomData ? `Allow to join the "${roomData.room.name}" room for everybody with the link.` : ""
        }
        checked={checked}

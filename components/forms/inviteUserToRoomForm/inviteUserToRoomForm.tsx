...
export const useInviteUserToRoomForm = ({
  ...
  const { mutate: inviteUser, ...mutationParams } = api.invitation.inviteUserToRoomMutation.useMutation();
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    inviteUser(
      ...
      {
        onError: (error) => {
          toast.error(error.message ?? "Failed to invite the user", { richColors: true });

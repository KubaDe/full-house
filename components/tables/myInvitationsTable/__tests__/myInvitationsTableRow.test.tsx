...
import { MyInvitationsTableRow } from "@/components/tables/myInvitationsTable/myInvitationsTableRow";
import { respondInvitationMutationMock } from "@/server/api/invitation/__mocks__/respondInvitationMutation.mock";
...
describe("MyInvitationsTableRow", () => {
  ...
  it("should display invitation row correctly", async () => {
    render(<MyInvitationsTableRow invitation={INVITATION} />, { wrapper: TableWrapper });
    ...
    expect(actionsButtons[0]).toHaveAttribute("aria-label", "Accept invitation");
    expect(actionsButtons[1]).toHaveAttribute("aria-label", "Reject invitation");
  });
  ...
  it("should call respondInvitationMutation when clicking accept button and confirming", async () => {
    ...
    expect(respondInvitationSpy).toHaveBeenCalledWith({ invitationId: "invitationId1", accept: true });
  });
  ...
  it("should call respondInvitationMutation when clicking reject button and confirming", async () => {
    ...
    await waitFor(() => expect(respondInvitationSpy).toHaveBeenCalled());
    expect(respondInvitationSpy).toHaveBeenCalledWith({ invitationId: "invitationId1", accept: false });
  });
});

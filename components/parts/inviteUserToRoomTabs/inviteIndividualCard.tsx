...
export const InviteIndividualCard = ({ roomId, onSuccess }: InviteIndividualCardProps) => {
  const {
    formUI,
    formName,
    form,
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{roomData ? `Invite single user to room "${roomData.room.name}"` : ""}</CardTitle>
        <CardDescription>
          Please provide the user's email address. The user will receive a notification regarding the pending
          invitation.
        </CardDescription>
      </CardHeader>

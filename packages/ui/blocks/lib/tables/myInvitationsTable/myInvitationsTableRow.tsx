...
import { TableCell, TableRow } from "@repo/ui-kit/table";
import { api } from "@repo/api/client";
import { Button } from "@repo/ui-kit/button";
import { type MyInvitationsQueryOutput } from "@repo/schemas";
...
export const MyInvitationsTableRow = ({
  invitation,
}: MyInvitationsTableRowProps) => {
  const utils = api.useUtils();
  const { mutateAsync: respondInvitation, isPending } =
    api.invitation.respondInvitationMutation.useMutation({
      onSuccess: (_, { accept }) => {

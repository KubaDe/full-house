...
describe("AddParticipantButton", () => {
  ...
  it("should render pending invitations counter for less than 9 invitations pending", async () => {
    ...
    expect(
      await screen.findByLabelText("pending invitations counter"),
    ).toBeVisible();
    expect(
      screen.getByLabelText("pending invitations counter"),
    ).toHaveTextContent("3");
  });
  ...
  it("should render hover card with emails of invited people", async () => {
    ...
    const invitationsBadges =
      screen.getByLabelText("invitations list").children;

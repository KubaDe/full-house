...
import { PersonBadge, type Profile } from "./personBadge";
import { preset } from "./pokerDeck";
...
export const PokerSeat = ({
  ...
  const presetItem =
    preset.find((presetItem) => presetItem.value === activeValue) ?? {};
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="relative">
        {state === SeatState.Empty && (
          <motion.div
            {...cardElevatorProps}
            initial={animations[SeatState.Empty]}
          />
        )}
        {showBadge && (
          <PersonBadge profile={profile} isOpen={state === SeatState.Empty} />
        )}
      </div>
    </div>
  );

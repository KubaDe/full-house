...
test.describe.serial("History messages", () => {
  test("Users send messages", async ({ participantA, participantB, room }) => {
    ...
    for (const i of range(4)) {
      for (const j of range(4)) {
        await participantA.page.getByTestId("chat.messageInput").fill(`User A - Message ${i * 8 + j + 1}`);
  ...
  test("Users C sees past conversation", async ({ participantC, room }) => {
    ...
    await test.expect(participantC.page.getByTestId("chat.message.content")).toHaveCount(20);
    await test
      .expect(participantC.page.getByTestId("chat.message.content").last())
      .toContainText(`User B - Message ${13}`);
    await test.expect(participantC.page.getByTestId("chat.loadMore")).toHaveAttribute("data-value", "12");

    await participantC.page.getByTestId("chat.loadMore").click();

    await test.expect(participantC.page.getByTestId("chat.message.content")).toHaveCount(32);
    await test
      .expect(participantC.page.getByTestId("chat.message.content").last())

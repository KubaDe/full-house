...
const test = base.extend<
  {},
  { participantA: Account; participantB: Account; participantC: Account; room: Room }
>({
  participantA: [
    async ({ browser }, use) => {
...
test.describe.serial("Send simple message", () => {
  test("Users A starts chat and sends message", async ({ participantA, room }) => {
    await participantA.page.goto(`/room/${room.stateHelpers.roomId}`);
    await participantA.page.getByTestId("roomSplitView.modeButton.chat").click();
    await participantA.page.getByTestId("chat.startChat").click();
    await participantA.page.getByTestId("chat.messageInput").fill("User A - Hello, world!");
    await participantA.page.getByTestId("chat.sendMessage").click();
    await test.expect(participantA.page.getByTestId("chat.message.content")).toHaveCount(1);
  });

  test("Users B sends message", async ({ participantB, room }) => {
    await participantB.page.goto(`/room/${room.stateHelpers.roomId}`);
    await participantB.page.getByTestId("roomSplitView.modeButton.chat").click();
    await test.expect(participantB.page.getByTestId("chat.message.content")).toHaveCount(1);
    await participantB.page.getByTestId("chat.messageInput").fill("User B - Hello, world!");
    await participantB.page.getByTestId("chat.sendMessage").click();
    await test.expect(participantB.page.getByTestId("chat.message.content")).toHaveCount(2);
  });

  test("Users C sends message", async ({ participantC, room }) => {
    await participantC.page.goto(`/room/${room.stateHelpers.roomId}`);
    await participantC.page.getByTestId("roomSplitView.modeButton.chat").click();
    await test.expect(participantC.page.getByTestId("chat.message.content")).toHaveCount(2);
    await participantC.page.getByTestId("chat.messageInput").fill("User C - Hello, world!");
    await participantC.page.getByTestId("chat.sendMessage").click();
    await test.expect(participantC.page.getByTestId("chat.message.content")).toHaveCount(3);
  });
});

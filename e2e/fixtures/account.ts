...
class StateHelpers {
  ...
  async clearAccount() {
    const user = await db.user.findFirstOrThrow({ where: { email: this.testUserEmail } });
...
export class Account {
  ...
  async init({ browser }: { browser: Browser }, { skipProfile = false }: { skipProfile?: boolean } = {}) {
    const context = await browser.newContext({ baseURL: process.env.NEXT_PUBLIC_BASE_URL });
    this._page = await context.newPage();
    await this.stateHelpers.createAccount();
    await this.login();
    const { id } = await db.user.findFirstOrThrow({ where: { email: this.testUserEmail } });
